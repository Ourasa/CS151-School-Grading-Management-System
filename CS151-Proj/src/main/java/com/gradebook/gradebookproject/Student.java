...
public class Student extends User {
	...
	public Student(String firstName, String lastName, String id, String password, TreeMap<Course, Character> curCourses,
			TreeMap<Course, Character> pastCourses) {
		super(firstName, lastName, id, password);
	...
	public void updateGPA() {
		...
		for (Map.Entry<Course, Character> set : pastCourses.entrySet()) {
			...
			if (set.getValue() == 'A') {
				...
				sum += 0;
			} else if (set.getValue() == 'N') { // 'N' is a "grade" that specifies no assignments were done. Course does
												// not count towards GPA.
				coursesCount -= 1;
			}
